...
router.patch('/', requireRole('ADMIN', 'SUPER_ADMIN'), validate({ body: z.object({ name: z.string().optional(), logoUrl: z.string().url().nullable().optional(), settings: z.record(z.unknown()).optional() }) }), auditLogger('org.update'), organizationsController.update);
