...
const postFnParams = z.object({ projectId: id, wfId: id, tId: id, pfId: id }).passthrough();
router.get('/', requireProjectRole(ProjectRole.MEMBER), validate({ params: projectParams }), workflowsController.list);
...
router.get('/:wfId', requireProjectRole(ProjectRole.MEMBER), validate({ params: workflowParams }), workflowsController.get);
...
router.post('/:wfId/transitions/:tId/post-functions', requireProjectRole(ProjectRole.ADMIN), validate({ params: transitionParams, body: workflowSchemas.postFn }), auditLogger('workflow.postfn.create'), workflowsController.createPostFn);
router.delete('/:wfId/transitions/:tId/post-functions/:pfId', requireProjectRole(ProjectRole.ADMIN), validate({ params: postFnParams }), auditLogger('workflow.postfn.delete'), workflowsController.removePostFn);
