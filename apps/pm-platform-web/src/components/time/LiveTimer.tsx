...
export function LiveTimer({ issueId }: { issueId: string }) {
  ...
  return <div className="rounded-xl border bg-card p-4">
    <div className="mb-3 flex items-center justify-between"><div className="text-sm font-semibold">Live timer</div><Button size="sm" variant="outline" onClick={() => setManualOpen(true)}>Log work</Button></div>
    ...
    <div className="flex flex-wrap gap-2">
      ...
      {timer?.status === 'ACTIVE' && <Button variant="outline" onClick={() => pause.mutate()}><Pause className="h-4 w-4" />Pause</Button>}
      {timer && <Button variant="destructive" onClick={handleStop} disabled={stop.isPending}><Square className="h-4 w-4" />Stop & save</Button>}
    </div>
    <p className="mt-3 text-xs text-muted-foreground">Stopping the timer saves a worklog automatically.</p>
    <Dialog open={manualOpen} onOpenChange={setManualOpen}><DialogContent><DialogHeader><DialogTitle>Log work</DialogTitle></DialogHeader><WorklogForm issueId={issueId} defaultSeconds={1800} onSaved={() => { setManualOpen(false); qc.invalidateQueries({ queryKey: queryKeys.issue(issueId) }); }} /></DialogContent></Dialog>
  </div>;
}
