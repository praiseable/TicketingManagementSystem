...
import { MessageSquare, Paperclip, Eye, Clock3, ExternalLink } from 'lucide-react';
...
export function IssueCard({ issue, preview = false }: { issue: Issue; preview?: boolean }) {
  ...
  const content = (
    ...
    >
      ...
      <div className="rounded-md p-1 transition group-hover:bg-primary/5">
        <h4 className="line-clamp-3 text-sm font-semibold leading-5 group-hover:text-primary">
          {issue.title}
        </h4>
      ...
      <div className="mt-3 flex items-center justify-between gap-2 border-t pt-3 text-xs text-muted-foreground">
        ...
        <div className="flex min-w-0 items-center gap-2">
          {typeof issue.storyPoints === 'number' && (
            <span className="rounded bg-muted px-1.5 py-0.5 font-medium">{issue.storyPoints} pts</span>
          )}
  ...
  return (
    <Link to={detailPath} className="block focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
      {content}
    </Link>
  );
}
